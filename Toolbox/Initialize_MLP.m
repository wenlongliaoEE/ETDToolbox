function varargout = Initialize_MLP(varargin)
% INITIALIZE_MLP MATLAB code for Initialize_MLP.fig
%      INITIALIZE_MLP, by itself, creates a new INITIALIZE_MLP or raises the existing
%      singleton*.
%
%      H = INITIALIZE_MLP returns the handle to a new INITIALIZE_MLP or the handle to
%      the existing singleton*.
%
%      INITIALIZE_MLP('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in INITIALIZE_MLP.M with the given input arguments.
%
%      INITIALIZE_MLP('Property','Value',...) creates a new INITIALIZE_MLP or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before Initialize_MLP_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to Initialize_MLP_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help Initialize_MLP

% Last Modified by GUIDE v2.5 30-Aug-2021 11:51:17

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @Initialize_MLP_OpeningFcn, ...
                   'gui_OutputFcn',  @Initialize_MLP_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before Initialize_MLP is made visible.
function Initialize_MLP_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to Initialize_MLP (see VARARGIN)

% Choose default command line output for Initialize_MLP
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes Initialize_MLP wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = Initialize_MLP_OutputFcn(hObject, eventdata, handles) 
